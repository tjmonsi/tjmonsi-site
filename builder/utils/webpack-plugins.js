...
const createHTMLOptions = require('./create-html-options');
const createManifest = require('./create-manifest');

module.exports = (dev) => {
  const environment = dev === 'prod' ? 'production' : 'development';
  const plugins = [
    new HTMLWebpackPlugin(createHTMLOptions(dev)),
